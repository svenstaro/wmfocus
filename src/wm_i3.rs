...
pub fn get_windows() -> Result<Vec<DesktopWindow>> {
    // Establish a connection to Sway or i3 over a Unix socket
    let mut connection = Connection::new().expect("Couldn't acquire Sway/i3 IPC connection");
...
pub fn focus_window(window: &DesktopWindow) -> Result<()> {
    let mut connection = Connection::new().expect("Couldn't acquire Sway/i3 connection");
